...
export HOST_MOUNT
HOST_MOUNT=$(bashio::config 'host_mount')
bashio::log.info "Host-Mount Modus: ${HOST_MOUNT}"
if [ "${HOST_MOUNT}" = "true" ]; then
    bashio::log.info "Mounts werden auf Host-Ebene ausgeführt (sichtbar für alle Addons unter /media/)"
else
    bashio::log.info "Mounts nur innerhalb dieses Containers sichtbar"
fi
