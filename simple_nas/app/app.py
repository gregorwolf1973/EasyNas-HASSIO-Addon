...
def _host_cmd(cmd):
    """Run a command in the host's mount namespace via nsenter (PID 1).
    This is required inside HA addon containers to access real block devices."""
...
def api_mount():
    ...
    # Create mountpoint on HOST via nsenter
    try:
        subprocess.run(
            _host_cmd(["mkdir", "-p", mountpoint]),
            capture_output=True, text=True, check=True
        )
    except subprocess.CalledProcessError:
        os.makedirs(mountpoint, exist_ok=True)  # fallback
