...
def _helper_call(action, arg1="", arg2="", arg3="", timeout=20):
    """Send a command to mount_helper.sh via FIFO and return (rc, output)."""
    import time

    # Wait up to 10s for FIFO to appear (helper may still be starting)
    deadline_fifo = time.time() + 10
    while not os.path.exists(MOUNT_FIFO):
        if time.time() > deadline_fifo:
            return 1, "Mount-Helper FIFO nicht gefunden – läuft mount_helper.sh?"
        time.sleep(0.2)
    ...
    try:
        ...
        return 1, f"Fehler beim Schreiben in FIFO: {e}"
    ...
    return 1, "Timeout: Mount-Helper hat nicht geantwortet"
...
def api_restart_samba():
    import time
    try:
        # Regenerate smb.conf before restart
        reload_samba()
        ...
        # FIX: --debug-stdout (not --log-stdout which doesn't exist)
        subprocess.Popen(["smbd", "--no-process-group", "--foreground", "--debug-stdout"])
        subprocess.Popen(["nmbd", "--no-process-group", "--foreground", "--debug-stdout"])
        time.sleep(1)

        # Verify they started
        import psutil
        smbd_ok = any(p.name() == "smbd" for p in psutil.process_iter(["name"]))
        nmbd_ok = any(p.name() == "nmbd" for p in psutil.process_iter(["name"]))

        if not smbd_ok or not nmbd_ok:
            return jsonify({"error": f"Start fehlgeschlagen: smbd={'OK' if smbd_ok else 'FEHLT'}, nmbd={'OK' if nmbd_ok else 'FEHLT'}"}), 500
