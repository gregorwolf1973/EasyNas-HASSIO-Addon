...
def _ns_cmd(cmd):
    """Run cmd in the host's mount namespace AND root filesystem.
    --mount=/proc/1/ns/mnt  → enter host mount namespace
    --root=/proc/1/root     → use host root (so /dev/sdb1 is the real block device)
    --wd=/                  → working directory in host root
    Requires full_access: true in config.yaml (gives CAP_SYS_ADMIN + /proc/1 access)
    """
    return [
        "nsenter",
        "--mount=/proc/1/ns/mnt",
...
def api_mount():
    ...
    # Create mountpoint in host namespace
    subprocess.run(_ns_cmd(["mkdir", "-p", mountpoint]), capture_output=True)
    os.makedirs(mountpoint, exist_ok=True)  # also locally for Samba
    ...
    try:
        result = subprocess.run(
            _ns_cmd(cmd), capture_output=True, text=True, check=True
        )
    except subprocess.CalledProcessError as e:
        err = e.stderr.strip() or e.stdout.strip() or "Mount fehlgeschlagen"
...
def api_unmount():
    ...
    try:
        subprocess.run(
            _ns_cmd(["umount", target]),
            capture_output=True, text=True, check=True
        )
    except subprocess.CalledProcessError as e:
        err = e.stderr.strip() or "Aushängen fehlgeschlagen"
        return jsonify({"error": err}), 500
