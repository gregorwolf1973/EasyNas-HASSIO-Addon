...
def _helper_call(action, arg1="", arg2="", arg3="", timeout=15):
    """Send a command to the privileged mount_helper.sh daemon via FIFO.
    ...
    try:
        # Open FIFO non-blocking to avoid hanging if helper died
        fd = os.open(MOUNT_FIFO, os.O_WRONLY | os.O_NONBLOCK)
        os.write(fd, cmd_line.encode())
        os.close(fd)
    except OSError as e:
        return 1, f"Mount-Helper nicht erreichbar: {e}"
    ...
    return 1, "Timeout: keine Antwort vom Mount-Helper"
